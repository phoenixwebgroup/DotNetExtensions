namespace BclExtensionMethods
{
	...
	{
		public static decimal? WeightedAverage<TSource>(this IEnumerable<TSource> source, Func<TSource, decimal?> valueSelector, Func<TSource, decimal?> weightSelector)
		{
			decimal? weightTotal = source.Select(weightSelector).Sum();

			decimal weightedSum = source.Sum(item => (valueSelector(item) ?? 0) * (weightSelector(item) ?? 0));
		...
		public static decimal? WeightedAverageValues<TSource>(this IEnumerable<TSource> source, Func<TSource, decimal?> valueSelector, Func<TSource, decimal?> weightSelector)
		{
			...
			return ((valueTotal.HasValue && weightTotal.HasValue && weightTotal.Value != 0m)
				? (valueTotal.Value / weightTotal.Value) : (decimal?)null);
		}
	}
}